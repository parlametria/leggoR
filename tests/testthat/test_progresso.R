...
setup <- function(){
  ...
  etapas_pec_15_2015 <<- list()
  etapas_pec_15_2015 %<>% append(list(process_etapa(1198512, "camara", as.data.frame())))
  etapas_pec_15_2015 %<>% purrr::pmap(dplyr::bind_rows)
  progresso_pec_15_2015 <<- get_progresso(etapas_pec_15_2015$proposicao, etapas_pec_15_2015$fases_eventos)
  return(TRUE)
}
...
test <- function() {
  test_that("get_progresso() returns dataframe", {
    ...
    expect_true(is.data.frame(progresso_pec_15_2015))
  })
  test_that("get_progresso() is not empty", {
    expect_true(nrow(progresso) != 0)
    expect_true(nrow(progresso_pec_15_2015) != 0)
  })
  ...
  test_that('progresso mpv', {
    ...
    progresso_gabarito_pec_15_2015 <-
      tibble::tribble(
    ...
    progresso_gabarito_pec_15_2015 <-
      progresso_gabarito_pec_15_2015 %>% dplyr::mutate(prop_id = as.integer(prop_id),
                                         data_fim = as.POSIXct(data_fim))

    expect_equal(progresso_pec_15_2015 %>% dplyr::mutate(data_inicio = as.character(data_inicio)), progresso_gabarito_pec_15_2015)
