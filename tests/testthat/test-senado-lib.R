...
test <- function(){
  ...
  test_that('extract_fase_Senado() is dataframe', {
    expect_true(is.data.frame(
      extract_fase_Senado(PROPOSICOES_ID, phase_one, phase_two, phase_three, phase_four)))
  })
  ...
  test_that('tail_descricao_despacho_Senado()', {
    expect_true(all(names(tail_descricao_despacho_Senado(fetch_tramitacao(91341)))
                    %in% .COLNAMES_DESPACHO_SEN))
  })

  test_that('extract_comissoes_Senado()', {
    expect_true(all(names(extract_comissoes_Senado(fetch_tramitacao(91341)))
                    %in% .COLNAMES_DESPACHO_SEN))
  })
