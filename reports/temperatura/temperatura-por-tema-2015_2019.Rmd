...
```{r}
plot_evol_temp_an <- evolucao_semanal_temperatura %>%
  filter(semana >= min_date,semana <= max_date,tema=="Agenda Nacional") %>%
  ggplot(aes(x=as.Date(semana), y=temperatura_recente)) +
  geom_line(fill=cor_temperatura) +
  xlab("Tempo") + 
  ylab("Temperatura") + 
  scale_y_sqrt() +
  scale_x_date(date_labels = "%d-%m-%Y") +
...
```{r warning=FALSE, message=FALSE}
semana <- data.frame(tema = "Agenda Nacional",
                     semana = lubridate::ymd(c('2018-06-18')
                                             , tz=lubridate::tz(temperatura_props_temas$semana[1])))
