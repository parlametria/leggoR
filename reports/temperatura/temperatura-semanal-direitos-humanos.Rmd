---
title: "Análise da Evolução Semanal da Temperatura - Direitos Humanos"
output:
  html_document:
    df_print: paged
...
temperatura_semanas_destaque_dh <- temperatura_temas %>%
  inner_join(semanas_destaque_dh) %>%
  group_by(semana,ano) %>%
  summarise(total_temp = sum(total_temp))

evolucao_temp_total_dh = temperatura_semanas_destaque_dh$total_temp[2] - 
  temperatura_semanas_destaque_dh$total_temp[1]
temp_total_semana_passada_dh = temperatura_semanas_destaque_dh$total_temp[1]
temp_total_semana_atual_dh = temperatura_semanas_destaque_dh$total_temp[2]

plot_temperatura_dh <- temperatura_semanas_destaque_dh %>% 
  ggplot(aes(x=as.Date(semana), y=total_temp)) +
  geom_col(position="dodge") +
  xlab("Tempo") + 
  ylab("Temperatura") +
...
```{r}
plot_temperatura_pls_dh <- evolucao_semanal_temperatura_dh %>%
...
ggplotly(plot_temperatura_pls_dh)
```


Vamos analisar quais os eventos que produziram a subida de temperatura dos PLs acima nessa semana.

### PEC 29/2015 -	PEC Antiaborto
```{r}
pl_destaque <- data.frame(id_ext = 120152,
                     semana = lubridate::ymd(c('2019-05-06')
                                             , tz=lubridate::tz(temperatura_props_temas$semana[1])))

eventos_semana_pec_29 <- eventos_semanais %>%
  inner_join(pl_destaque)

eventos_semana_pec_29
```
```{r}
eventos_resumidos_semana_pec_29 <- eventos_semana_pec_29 %>%
  inner_join(pl_destaque) %>%
...
eventos_resumidos_semana_pec_29
```


Texto para o report: 

Na semana passada (06-10/05/2019), o tema de Direitos Humanos houve queda de temperatura de `r abs(evolucao_temp_total_dh)` unidades, indo de  `r temp_total_semana_passada_dh` para `r temp_total_semana_atual_dh`, como pode ser visto no gráfico abaixo `r ggplotly(plot_temperatura_dh)`.

As proposições em atividade nesse período foram, em ordem decrescente de aumento da temperatura:
`r ggplotly(plot_temperatura_pls_dh)`.

Principais atualizações:

Na PEC Antiaborto (PEC 29/2015), a matéria foi retirada de pauta e devolvida à relatora Senadora Juíza Selma. [Mais informações](leggo.org.br)