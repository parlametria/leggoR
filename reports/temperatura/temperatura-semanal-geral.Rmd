---
title: "Análise da Evolução Semanal da Temperatura - Meio Ambiente"
output:
  html_document:
    df_print: paged
...
Vejamos agora de forma resumida (agregada) os eventos reconhecidos na semana
```{r}
resumo_eventos_semana_mpv_870_2019 <- eventos_semana_mpv_870_2019 %>%
  filter(!is.na(titulo_evento)) %>%
  group_by(titulo_evento) %>%
  summarise(num_eventos = n()) %>%
  arrange(desc(num_eventos))

resumo_eventos_semana_mpv_870_2019
```

## Texto para o report: 

Na semana passada (06-10/05/2019), as proposições acompanhadas pelo Leggo tiveram um acréscimo de temperatura total de `r evolucao_temp_total_tema` unidades, indo de  `r temp_total_semana_passada_tema` para `r temp_total_semana_atual_tema`, como pode ser visto no gráfico abaixo. `r ggplotly(plot_temperatura_tema)`
...
Na Reforma da Previdência (PEC 6/2019) - Agenda Nacional:
...
Na PEC 15/2015 - Tornar o FUNDEB permanente (Câmara) - Educação:
...
Na MPV de Monitoramento e Acompanhamento de ONGs (MPV 870/2019) - Agenda Nacional:
