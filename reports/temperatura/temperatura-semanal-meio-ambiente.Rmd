...
library(ggplot2)
library(plotly)
semanas_destaque_tema <- data.frame(tema = "Meio Ambiente",
...
### PL 9950/2018 - Lei do Pantanal - Molon

Vejamos, para a proposição atual, quais os eventos ocorridos na semana
```{r}
pl_destaque <- data.frame(id_ext = 2170839,
                     semana = lubridate::ymd(c('2019-05-06')
                                             , tz=lubridate::tz(temperatura_props_temas$semana[1])))

eventos_semana_pl_9950_2018 <- eventos_semanais %>%
  inner_join(pl_destaque) %>%
  select(data,sequencia,descricao_situacao,titulo_evento,texto_tramitacao)

eventos_semana_pl_9950_2018
```

Vejamos agora de forma resumida (agregada) os eventos reconhecidos na semana
```{r}
resumo_eventos_semana_pl_9950_2018 <- eventos_semana_pl_9950_2018 %>%
  filter(!is.na(titulo_evento)) %>%
  group_by(titulo_evento) %>%
  summarise(num_eventos = n()) %>%
  arrange(desc(num_eventos))

resumo_eventos_semana_pl_9950_2018
```

## Texto para o report: 

Na semana passada (06-10/05/2019), o tema Meio Ambiente teve um acréscimo de temperatura total de `r evolucao_temp_total_tema` unidades, indo de  `r temp_total_semana_passada_tema` para `r temp_total_semana_atual_tema`, como pode ser visto no gráfico abaixo. `r ggplotly(plot_temperatura_tema)`
...
No PL 9950/2018 - Lei do Pantanal - Molon, o relator Deputado Nilto Tatto (PT-SP) apresentou na CMADS um Parecer pela Aprovação, com emendas. [Mais informações](https://leggo.org.br)