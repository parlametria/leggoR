...
compute_nodes_size <- function(final_edges, final_nodes, smoothing) {
  nodes_size_source <-
    final_edges %>%
...
generate_nodes_and_edges <- function(coautorias, edges_weight = 100, smoothing = 1) {
  ...
  final_nodes <- graph_nodes %>%
    as.data.frame() %>%
    mutate(nome_eleitoral = agoradigital::formata_nome_eleitoral(nome, partido, uf))
  ...
  final_nodes <-
    compute_nodes_size(final_edges, final_nodes, smoothing)
