...
params:
  ...
  temperaturas: 
    label: "Input dataset:"
    value: ../../data/hists_temperatura.csv
    input: file
---
...
temperatura <- read_csv(params$temperaturas)
```


```{r, message=FALSE, warning=FALSE, include=FALSE}
#Base de dados de pressão que tiveram em pauta
pressao_137999 <- read_csv("./data/pop_137999.csv")
pressao_2192459 <- read_csv("./data/pop_2192459.csv")
pressao_prev <- bind_rows(pressao_137999, pressao_2192459)
pressao_cp <- read_csv("./data/pop_16526.csv") 
...
atores_ultima_semana <-
  atores_geral %>%  
  filter(data >= params$date_init) %>% 
  filter(data < params$date_end)
...
pauta_prop <- function(temperatura_df, pressao_df) {
  ...
  pauta <- inner_join(temperatura_, pressao_, by = "periodo")
  return(pauta)
}
```
...
```{r}
pauta_prev <- pauta_prop(temperatura_pauta_prev, pressao_prev)
pauta_cp <- pauta_prop(temperatura_pauta_cp, pressao_cp)
```
...
```{r}
pauta_137999 <- sumariza_prop(pauta_prev)
pauta_16526 <- sumariza_prop(pauta_cp)
```
...
Para a última semana, a PL 1292/1995 não tivemos evento.
