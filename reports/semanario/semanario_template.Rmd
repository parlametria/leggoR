---
title: "Destaques da Semana - Leg.go"
output:
  html_document:
    css: style.css
params:
...
get_texto_temp_pressao <- function(temp_pressao_periodo, eventos_pl) {
  ...
  nivel_subida_temp <- if_else(variacao_temp > 100, "considerável", "")
  
  return (paste0("Foi observad", if(cresceu_temp){"o um crescimento "}else{"a uma diminuição "}, nivel_subida_temp, " na temperatura e ", if(cresceu_press){"um crescimento de "}else{ if(variacao_press == 0){"não foi observada nenhuma mudança"}else{"uma diminuição de "}}, if(variacao_press != 0){variacao_press}, if(variacao_press != 0){"%"}, " na pressão.", if(nrow(eventos_pl) > 0){paste0(" Os ", nrow(eventos_pl), " eventos que influenciaram a Temperatura estão na tabela abaixo.")}))
}
...
build_temp_pressao_plot <- function(temp_pressao_pl, apelido_pl, dt_inicio, dt_fim, eventos) {
  ...
  if (nrow(eventos_pl) == 0) {
    cat("Os eventos que fizeram a temperatura subir ainda não são reconhecidos pelo Leggo.")    
  } else {
    cat("\n\n")
    cat("#### ")
    print(knitr::kable(eventos_pl))
    cat("\n\n")
    cat(paste0("Para mais informações sobre os eventos e sobre a proposição, como o progresso, a rede de influência e uma análise sobre as emendas, [clique aqui](", link_para_leggo, ")."))
  }
  cat("\n\n")
}
...
build_atores_plot_bancadas_por_proposicao_tema <- function(atuacao_bancadas, tema) {
  p <- atuacao_bancadas %>% 
    ggplot(aes(x = reorder(apelido, peso_total_documentos), y = peso_total_documentos, group = bancada, fill = bancada)) + 
    geom_chicklet(width = 0.75) +
     theme_ipsum_rc(grid=FALSE) +
     theme(legend.position = "bottom") +
     labs(
    ...
    ggplot(aes(x = reorder(nome_completo, total_docs), y = peso_total_documentos, group = tipo_generico, fill = tipo_generico)) + 
      geom_chicklet(width = 0.8) +
       ...
       labs(
        ...
        subtitle = atores_pls_plot$apelido,
        caption = ""
       ) +
       scale_fill_manual(
       name = NULL,
...
if (nrow(atores) == 0) {
  ...
  atuacao_bancadas_por_pl_tema <- atores %>% 
    dplyr::group_by(tema, id_leggo, casa, bancada, nome_formal, nome_formal_completo, sigla_tipo, apelido) %>% 
    dplyr::summarise(peso_total_documentos = sum(peso_total_documentos))
