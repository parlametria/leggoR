...
emoji <- atores_pls %>% ungroup() %>% mutate(nome_completo = if_else(bancada == "Oposição")
p <- emoji %>% ggplot(aes(x = reorder(nome_completo, qtd_de_documentos), y = qtd_de_documentos, group = tipo_generico, fill = tipo_generico)) + 
