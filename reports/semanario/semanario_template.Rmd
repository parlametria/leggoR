...
build_temp_pressao_plot <- function(temp_pressao_pl, apelido_pl) {
  p <- temp_pressao_pl %>% 
    ggplot(aes(x = periodo, y = sqrt(temperatura))) +
    geom_line(color="#DDDDDD") +
    geom_point(color=factor(ifelse(temp_pressao_pl$periodo==data_inicio,"#EF476F","#DDDDDD")),
             size = ifelse(temp_pressao_pl$periodo==data_inicio,3,1),
             width=0.5) +
    geom_col(aes(x = periodo, y = -sqrt(pressao), width = 4), fill=factor(ifelse(temp_pressao_pl$periodo==data_inicio,"#FFD166","#DDDDDD")), show.legend = FALSE ) +
    labs(x = "Semana", 
         y = "",
...
build_temp_pressao_plot(temp_pressao_periodo, "PLP 38/2019 - Pacote Moro Anticorrupção")
...
atores_geral <- atores %>% 
  ...
  left_join(pls_nome_formal %>% select(id_leggo, id_ext, casa, nome_formal), by=c("id_leggo","id_ext","casa")) %>% 
  mutate(bancada = if_else(partido %in% oposicao, "Oposição", "Governo"))

atores_pls <- atores_geral %>% 
  dplyr::group_by(id_leggo, id_ext, casa, nome_autor, casa, id_autor, tipo_autor, partido, nome_completo,
...
join_temp_pressao <- function(temperatura_df, pressao_df) {
  ...
  temp_pressao <- dplyr::inner_join(temperatura_, pressao_df, by =c("id_leggo","periodo")) %>% 
    ...
    mutate(pressao = if_else(pressao == 0, 2, pressao))
  return(temp_pressao)
}
```
...
build_temp_pressao_plot(temp_pressao_periodo, paste(pl_destaque$nome_formal,pl_destaque$apelido,sep=" - "))
