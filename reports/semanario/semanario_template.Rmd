...
build_temp_pressao_plot <- function(temp_pressao_pl, apelido_pl, dt_inicio, dt_fim) {
  ...
  p <- temp_pressao_pl %>% 
    ...
    geom_point(color=factor(ifelse(temp_pressao_pl$periodo>data_n_semanas_atras,"#EF476F","#DDDDDD")),
...
build_temp_pressao_plot(temp_pressao_periodo, paste(pls_destaque$nome_formal,pls_destaque$apelido,sep=" - "), data_inicio, data_fim)
```
...
#  print(paste("Rodando iteração ",i))
  ...
  #build_temp_pressao_plot(temp_pressao_pl, paste(pl_destaque$nome_formal,pl_destaque$apelido,sep=" - "), data_inicio, data_fim)
#}
```
...
`r if (nrow(temperatura) == 0){"Na última semana não observamos nenhum crescimento expressivo na temperatura de nenhuma proposição."}else{"Foi observad"}``r if(cresceu_temp){"o, na última semana, um crescimento de "}else{"a, na última semana, uma diminuição de "}` `r variacao_temp`% na temperatura e `r if(cresceu_press){"um crescimento de "}else{ if(variacao_press == 0){"não foi observada nenhuma mudança"}else{"uma diminuição de "}}` `r if(variacao_press != 0){variacao_press}``r if(variacao_press != 0){"%"}` na pressão.
...
for (i in 1:nrow(props_destaque)) {
  print(paste("Rodando iteração ",i))
  curr_prop <- props_destaque %>%  slice(i)
  curr_prop_nome_formal_completo <- curr_prop$nome_formal_completo
  ...
  #pl_chunk_label <- paste('atores',curr_prop_destaque$id_ext,curr_prop_destaque$casa,sep='-')
  #knitr::opts_chunk$set(label = pl_chunk_label)
  out = c(out, knitr::knit_child('atores_semanario_template.Rmd'))
}
```
