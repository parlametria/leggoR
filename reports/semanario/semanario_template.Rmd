...
get_texto_temp_pressao <- function(temp_pressao_periodo) {
...
build_temp_pressao_plot <- function(temp_pressao_pl, apelido_pl, dt_inicio, dt_fim) {
  ...
  texto_temp_pressao <- get_texto_temp_pressao(temp_pressao_pl)
...
get_texto_atores_plot_bancadas_por_temas <- function(qtd_tema, atuacao_temas, atores, tema_destaque) {
  ...
  num_docs_oposicao <- atuacao_temas_destaque %>% filter(bancada == "Oposição") %>% purrr::pluck("peso_total_documentos")
    ...
    return (paste0("Na última semana o tema ", tema_destaque, " teve destaque nas atuações parlamentares, sendo ", variacao_temp, "% dos documentos autorados para esse tema. Outro destaque é que foram apresentados ", num_docs_governo, " documentos pela bancada do governo e ", num_docs_oposicao, " pela oposição."))
}
...
build_atores_plot_bancadas_por_temas <- function(atuacao_temas) {
  ...
  return (atores %>%  filter(tema == tema_destaque))
}
...
get_texto_atores_plot_bancadas_por_proposicao <- function(qtd_atuacao, atuacao_bancadas, prop_destaque) {
  ...
  return (descricao_plot_atores <- paste0("A ", prop_destaque, " teve maior número de atuações, com apresentação de ", total_prop_destaque, " documentos, sendo ", num_docs_governo, " apresentados pelo governo e ",num_docs_oposicao, " pela oposição."))
}

build_atores_plot_bancadas_por_proposicao <- function(atuacao_bancadas, pls_tema_destaque) {
  p <- atuacao_bancadas %>% filter(nome_formal %in% pls_tema_destaque$nome_formal) %>%  ggplot(aes(x = reorder(nome_formal, peso_total_documentos), y = peso_total_documentos, group = bancada, fill = bancada)) + 
    geom_chicklet(width = 0.75) +
     theme_ipsum_rc(grid=FALSE) +
     theme(legend.position = "bottom") +
     labs(
...
build_atores_plot_por_proposicao <- function(atores_prop_destaque, prop_destaque) {
  cat(paste0("\nAo clicar na proposição ", prop_destaque, " temos mais detalhes sobre os atores e os respectivos documentos apresentados."))
  atores_pls_bancada <- 
  ...
  p <- atores_pls_plot %>% 
    ggplot(aes(x = reorder(nome_completo, total_docs), y = peso_total_documentos, group = tipo_generico, fill = tipo_generico)) + 
      geom_chicklet(width = 0.8) +
       ...
       labs(
        x = NULL, y = "Quantidade de documentos", fill = NULL,
        title = paste0("Atuações por proposição - ",atores_pls_plot$nome_formal_completo),
        subtitle = "Documentos autorados pelos parlamentares \nna última semana",
        caption = ""
       ) +
       scale_fill_manual(
  ...
  descricao_plot_atores <- paste0("O documento que os parlamentares mais apresentaram na ",
                                  atores_pls_plot$nome_formal_completo,
                                  " foi: ",
                                  tipo_generico_destaque)
...
if (nrow(pls_destaque) == 0) {
  ...
  res <- temp_pressao_periodo %>% 
   left_join(pls_destaque %>% select(id_leggo, nome_formal, apelido), by = "id_leggo") %>% 
    group_by(id_leggo) %>%
    group_map(~ build_temp_pressao_plot(., paste(.$nome_formal, .$apelido,sep=" - "), data_inicio, data_fim))
}
```

## Atores da semana

```{r atores_prop_destaque, fig.height=6, fig.width=10}
if (nrow(atores) < 5) {
  ...
  atuacao_temas <- atores %>% 
    dplyr::group_by(bancada, tema) %>% 
    dplyr::summarise(peso_total_documentos = sum(peso_total_documentos))
  
  atuacao_bancadas <- atores %>% 
    dplyr::group_by(id_leggo, casa, bancada, nome_formal, nome_formal_completo, sigla_tipo) %>% 
    dplyr::summarise(peso_total_documentos = sum(peso_total_documentos))
  
  pls_tema_destaque <- build_atores_plot_bancadas_por_temas(atuacao_temas)
  
  props_list <- build_atores_plot_bancadas_por_proposicao(atuacao_bancadas, pls_tema_destaque)
  ...
  res <- atores %>% 
  group_by(id_leggo) %>%
    group_map(~ build_atores_plot_por_proposicao(., prop_destaque))
}
```