...
plot_prop_comissoes <- function(id, name) {
  atores_prop <- 
    ...
    dplyr::summarise(total_eventos_ator_tipo_gen = sum(qtd_de_documentos)) %>% 
    dplyr::mutate(autor = ifelse(is.na(ultimo_status_sigla_uf),
                                 paste(nome_autor, " - ", substring(ultimo_status_sigla_partido, 0, 4)),
                                 paste(nome_autor, " - ", substring(ultimo_status_sigla_partido, 0, 4), "/", ultimo_status_sigla_uf)))
