...
extract_events_in_camara <- function(tramitacao_df) {
  regex_df <- camara_codes$eventos %>% select(-code) %>% filter(!is.na(regex))
  code_df <- camara_codes$eventos %>% select(-regex) %>% filter(!is.na(code))
  
  tramitacao_df %>%
    fuzzyjoin::regex_left_join(code_df, by = c(id_tipo_tramitacao = "code")) %>%
    fuzzyjoin::regex_left_join(regex_df, by = c(despacho = "regex"), ignore_case = TRUE) %>%
    mutate(evento = if_else(is.na(evento.x), evento.y, evento.x)) %>%
    select(-code, -regex, -evento.x, -evento.y)
}
