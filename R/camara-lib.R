...
get_comissoes_camara <- function() {
  ...
  dplyr::tibble(
    siglas_comissoes_antigas = list(c$siglas_comissoes_antigas),
  ...
  df %>%
    dplyr::mutate(relator = dplyr::case_when(
      stringr::str_detect(tolower(despacho), '^designad. relat.r') ~
        stringr::str_extract(despacho, stringr::regex('dep.+', ignore_case =
                                                        TRUE))
    ))
}
...
extract_autor_in_camara <- function(prop_id) {
  ...
  authors <- json_voting %>%
    ...
    dplyr::mutate(
      casa_origem =
        dplyr::case_when(
          stringr::str_detect(tolower(autor.nome), camara_exp) |
            autor.tipo == 'Deputado' ~ 'Câmara dos Deputados',
          stringr::str_detect(tolower(autor.nome), senado_exp) |
            autor.tipo == 'Senador' ~ 'Senado Federal',
  ...
  partido_estado <-
    extract_partido_estado_autor(authors$autor.uri %>% tail(1))
...
extract_locais_in_camara <- function(df) {
  ...
  df %<>%
        ...
        dplyr::case_when(
          (
            tolower(despacho) %in% descricoes_plenario & sigla_orgao == 'PLEN' |
              stringr::str_detect(tolower(descricao_tramitacao), '^votação')
          ) ~ 'Plenário',
          (
    ...
    dplyr::mutate(local =
                    dplyr::case_when(
                      stringr::str_detect(local, "^PL") ~ "Comissão Especial",
                      TRUE ~ local
                    ))
...
extract_situacao_comissao <- function(df) {
  ...
  analise_do_relator <-
    camara_codes$situacao_comissao$code$analise_do_relator
  discussao_votacao <-
    camara_codes$situacao_comissao$code$discussao_votacao
  encaminhamento <-
    camara_codes$situacao_comissao$code$encaminhamento
  ...
  df %>%
    dplyr::mutate(comissao =
                    dplyr::case_when(str_detect(local, reg) ~ str_extract(local, reg))) %>%
    # dplyr::filter(!is.na(comissao)) %>%
    dplyr::mutate(
      situacao_comissao =
        dplyr::case_when(
          id_tipo_tramitacao %in% recebimento &
            !is.na(comissao) ~ "Recebimento",
          id_tipo_tramitacao %in% analise_do_relator &
            !is.na(comissao) ~ "Análise do relator",
          id_tipo_tramitacao %in% discussao_votacao &
            !is.na(comissao)  ~ "Discussão e votação",
          id_tipo_tramitacao %in% encaminhamento &
            !is.na(comissao) ~ "Encaminhamento"
        )
    ) %>%
