...
#' @export
fetch_composicao_comissao <- function(sigla, casa, orgaos_camara, orgaos_senado, data_inicio_arg = "2019-02-01") {
  ...
  if (casa == 'camara') {
    ...
    new_name <- c("cargo", "id", "partido", "uf", "situacao", "nome", "data_inicio", "sigla", "casa")
    comissao <-
      fetch_composicao_comissoes_senado(sigla, orgaos_senado)
...
#' @return Dataframes
fetch_composicao_comissoes_senado <- function(sigla_arg, orgaos_senado) {
  tryCatch(
    {
      ...
      colegiado <-
        json_sessions %>%
        magrittr::extract2('UltimaComposicaoComissaoSf') %>%
        magrittr::extract2('ComposicaoComissao') %>%
        magrittr::extract2('Membros') %>% 
        magrittr::extract2('Membro')
      
      if (!is.null(colegiado)) {
        colegiado <- colegiado %>% 
          dplyr::mutate(CARGO = TipoVaga,
                        PARTIDO = NA,
                        UF = NA,
                        FOTO = NA) %>% 
          dplyr::select(ID = CodigoParlamentar,
                        CARGO,
                        PARTIDO,
                        UF,
                        TIPO_VAGA = TipoVaga,
                        PARLAMENTAR.x = NomeMembro,
                        FOTO,
                        DATA_INICIO = DataInicioMembroVaga)
        return(colegiado)
      } else {
        return(
          tibble::tibble(
...
fetch_all_composicao_comissao <- function(data_inicio = "2019-02-01") {
  ...
  siglas_comissoes <-
    dplyr::bind_rows(siglas_camara, siglas_senado, siglas_cong_nacional) %>%
    dplyr::distinct() %>%
    dplyr::arrange(casa,sigla)

  composicao_comissoes <-
    purrr::map2_df(siglas_comissoes$sigla, siglas_comissoes$casa, ~ fetch_composicao_comissao(.x, .y, orgaos_camara, orgaos_senado, data_inicio)) %>%
