...
onde:  
  ...
  - $e$ é a energia recente do período;  
...
Vamos calcular a energia recente para todo o histórico da tramitação do PL, agregando os eventos por semana e utilizando um decaimento de 10%. Os resultados podem ser vistos no gráfico abaixo.

#### Energia Semanal (pontos)
...
hist_energia_semanal <- agoradigital::get_historico_energia_recente(proc_tram, granularidade = 's', decaimento = 0.05)

library(ggplot2)
ggplot(hist_energia_semanal, aes(x=periodo, y=energia_recente)) +
  xlab("Tempo") + ylab("Energia") +
  geom_point() +
  theme_minimal()
```

#### Energia Semanal (linha)

Para ter uma visualização menos poluída, vamos mostrar uma linha, ao invés dos pontos.

```{r, warning=FALSE, message=FALSE, fig.width=7, fig.align='center'}
hist_energia_semanal %>% 
  ggplot2::ggplot(aes(x=periodo, y=energia_recente)) +
  ggplot2::geom_line() +
  ggplot2::xlab("Tempo") + 
...
#### Energia Semanal (curva suavizada)
...
#### Energia Semanal (curva suavizada - últimos 3 meses)
...
hist_energia_semanal_3_meses %>% 
  ggplot(aes(x=periodo, y=energia_recente)) +
  geom_smooth(span = .3, se = F) +
  geom_point(aes(x=periodo, y=energia_periodo), colour='red', alpha=0.5, subset(hist_energia_semanal_3_meses, energia_periodo > 0)) +
  xlab("Tempo") + 
  ylab("Energia") +
  theme_minimal()
```