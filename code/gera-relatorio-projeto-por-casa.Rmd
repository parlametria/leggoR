...
ultimo_relator <- function(){
  if(!identical(data$ultimo_relator[[1]], character(0))){
    paste0(data$ultimo_relator, ' - ',
      ifelse(
        identical(data$partido_relator[[1]], character(0)) | identical(data$uf_ultimo_relator[[1]], character(0)),
        '',
        paste0(data$partido_relator, '/', data$uf_ultimo_relator)
      )
    )
  }else {
    'Não encontrado'
